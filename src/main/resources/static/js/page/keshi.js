        ...
        for (var i = 0; i < medicals.length; i++) {
            ...
            for (var j = 0; j < koffices.length; j++) {
                ...
                if ((medicals[i].id) == (koffices[j].medicalId)) {
                    ...
                    $str2 = $("<ul><li><h3>" + koffices[j].kName + "</h3>" +
                        "<p style=\"display:-webkit-box; -webkit-line-clamp:3; -webkit-box-orient:vertical; overflow:hidden;\">" + koffices[j].resume + "</p>" +
                        "<div><a class=\"coffee\" href=\"kofficeById?id=" + koffices[j].id + "\" title=\"呼吸内科\">查看详细</a>" +
                        "<a class='green' href=\"doctorByKId?id=" + koffices[j].id + "\" title='呼吸内科医生'>科室医生</a></div></li></ul>");
                    $str.append($str2);
                }
            }
